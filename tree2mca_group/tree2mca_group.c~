  ...
  if(argc!=2)
    {
      ...
      printf("-----------------------\nSorts TIGRESS add-back spectra gated by TIGRESS-CsI Doppler shift groups defined via a group map.\n\n");
      exit(-1);
    }
    
  readConfigFile(argv[1],"tree2mca_group"); //grab data from the parameter file
...
void addTreeDataToOutHist()
{
  ...
  for (int i=0;i<stree->GetEntries();i++)
      ...
      for(int j=0;j<sortLeaf->GetLen();j++)
	{
	  ...
	  /* printf("pos %2d col %1d E %.3f w %.3f csi %2d E %.3f\n",pos,col,sort_value,weight_value,csi,csiE); */
	  /* getc(stdin); */
	  ...
	  **************************************************************/
	  ...
	  /* if(csi != 23) */
	      ...
	      if(sort_value >= 0.0)
		{
		  ...
		  if(histVal>=0.0)
		    if(histVal<S32K)
		      dOutHist[group][(int)(histVal)]+=(float)weight_value; //fill the output histogram
