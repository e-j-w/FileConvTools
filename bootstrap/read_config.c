void readConfigFile(const char * fileName,const char *configType) 
{
  ...
  while(!(feof(config)))//go until the end of file is reached
    {
	  ...
	  if(index<NSPECT)
	    // spectrum, channel range and step function parameter data
            if(sscanf(cfgstr,"%i %i %i %lf %lf",&spectrum[index],&startCh[index],&endCh[index],&sfc[index],&sfw[index])==5) 
              {
	      ...
	      if(strcmp(str1,"NUM_COMPARE")==0)
                numCompare=atoi(str2);
              if(strcmp(str1,"NUM_ENTRIES")==0)
                numEntries=atoi(str2);
  ...
  //Report parameters based on the config file type used
    if(strcmp(configType,"bootstrap")==0)
    {
      if(listMode==false)
	...
	printf("Will compare spectrum %i from channels %i to %i.\n",spectrum[index],startCh[index],endCh[index]);
      for(index=0;index<numSpectra;index++)
	printf("Will fit step function for spectrum %i using centroid %.2lf and width %.2lf\n",spectrum[index],sfc[index],sfw[index]);
      if(addBackground==0)
	printf("Will not add background to simulated data.\n");
      if(addBackground==1)
	printf("Will add background to simulated data.\n");
      if(plotOutput==0)
